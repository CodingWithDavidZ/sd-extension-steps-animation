...
temp_files = []
log = logging.getLogger("steps-animation test")
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):  # pylint: disable=unused-argument
        with gr.Accordion(
            "Steps animation test", open=False, elem_id="steps-animation test"
        ):
            gr.HTML(
                """
            ...
            with gr.Row():
                ...
                codec = gr.Radio(
                    label="Codec",
                    choices=["x264", "x265", "vpx-vp9", "aom-av1", "prores_ks"],
                    value="x264",
                )
                interpolation = gr.Radio(
                    label="Interpolation",
                    choices=["none", "mci", "blend"],
                    value="blend",
                )
            with gr.Row():
                duration = gr.Slider(
                    label="Duration", minimum=0.5, maximum=120, step=0.1, value=10
                )
                skip_steps = gr.Slider(
                    label="Skip steps", minimum=0, maximum=100, step=1, value=0
                )
            with gr.Row():
                last_frame_duration = gr.Slider(
                    label="Additional duration of the last frame",
                    minimum=00,
                    maximum=10,
                    step=1,
                    value=0,
            ...
            with gr.Row():
                preview_frame_duration = gr.Slider(
                    label="Prepend last frame to the beginning",
                    minimum=00,
                    maximum=10,
                    step=1,
                    value=0,
            ...
            with gr.Row():
                tmp_path = gr.Textbox(
                    label="Intermediate files path", lines=1, value="intermediate"
                )
                out_path = gr.Textbox(
                    label="Output animation path", lines=1, value="animation"
                )

        debug.change(
            fn=lambda: log.setLevel(logging.DEBUG)
            if debug.value
    ...
    def process(
        ...
        if is_enabled:
            ...
            if orig_callback_state == "undefined":
                log.debug(
                    f"Steps animation patching sampler callback for: {p.sampler_name}"
                )
            ...
            if shared.opts.data["show_progress_type"] != "Full":
                ...
                log.info(
                    f"Steps animation setting preview type to Full (current {shared.opts.data['show_progress_type']})"
                )
            ...
            def callback_state(self, d):
                ...
                for batch in range(0, p.batch_size):
                    ...
                    if (skip_steps == 0) or (current_step > skip_steps):
                        log.debug(
                            f"Steps animation saving interim image: step={current_step} batch={batch} iteration={p.iteration}: {fn}.{ext}"
                        )
                        try:
                            # latent = d['denoised'] if 'denoised' in d else d # shared.state.current_latent
                            latent = (
                                d if isinstance(d, torch.Tensor) else d["denoised"]
                            )  # shared.state.current_latent
                            image = sample_to_image(
                                samples=latent, index=batch % p.batch_size
                            )
                            ...
                            log.error(
                                f"Steps animation error: save intermediate image: {e}"
                            )
                return res
            ...
            if orig_callback_state != "undefined":
                if p.sampler_name in ["DDIM", "PLMS", "UniPC"]:
                    setattr(
                        VanillaStableDiffusionSampler, "update_step", callback_state
                    )
                else:
    ...
    def postprocess(
        ...
        def check_codec(
            ...
            lines = [
                line.strip()
                for line in lines
                if line.strip().startswith("V") and "=" not in line
            ]
        ...
        if orig_callback_state != "undefined":
            log.debug(
                f"Steps animation restoring sampler callback for: {p.sampler_name}"
            )
            if p.sampler_name in ["DDIM", "PLMS", "UniPC"]:
        ...
        if current_step == 0:
            log.error(
                "Steps animation error: steps is zero, likely using unsupported sampler or interrupted"
            )
            return
        ...
        if vars(processed)["steps"] < vars(processed)["steps"]:
            log.debug(
                f'Steps animation warning: postprocess early call: current={vars(processed)["steps"]} target={vars(processed)["steps"]}'
            )
        ...
        params = {
            ...
            "model": v["info"].split("Model:")[1].split()[0]
            if ("Model:" in v["info"])
            else "unknown",  # parse string if model info is present
            "embedding": v["info"].split("Used embeddings:")[1].split()[0]
            if ("Used embeddings:" in v["info"])
            else "none",  # parse string if embedding info is present
            ...
            "preview_frame_duration": preview_frame_duration,
            "interpolation": interpolation,
            "loglevel": "error",
            "cli": cli_template,
            "framerate": max(
                0, 1.0 * (current_step - skip_steps) / duration
        ...
        for iteration in range(0, params["batchcount"]):
            for batch in range(0, params["batchsize"]):
                ...
                log.debug(
                    f'Steps animation processing batch={batch + 1}/{params["batchsize"]} iteration={iteration + 1}/{params["batchcount"]}'
                )
                params["seed"] = v["all_seeds"][index]
                params["prompt"] = safestring(v["all_prompts"][index])
                params["short_name"] = (
                    str(params["seed"]) + "-" + params["prompt"][:name_length]
                )
                params["outfile"] = unique_filename(
                    os.path.join(params["outpath"], params["short_name"]), suffix
                )
                params["sequence"] = f"{iteration:02d}{batch:02d}{(skip_steps + 1):03d}"
                params[
                    "description"
                ] = "{prompt} | negative {negative} | seed {seed} | sampler {sampler} | cfgscale {cfgscale} | steps {steps} | last {laststep} | model {model} | embedding {embedding} | faces {faces} | timestamp {timestamp} | interpolation {interpolation}".format(
                    **params
                )
                ...
                if debug:
                    ...
                    log.debug(
                        f"Steps animation params: {json.dumps(params, indent = 2)}"
                    )
                if out_create:
                    imgs = [
                        f
                        for f in os.listdir(params["inpath"])
                        if f.startswith(f"{iteration:02d}{batch:02d}")
                        and params["short_name"][:16] in f
                    ]
                    ...
                    if not os.path.isdir(params["outpath"]):
                        ...
                        pathlib.Path(params["outpath"]).mkdir(
                            parents=True, exist_ok=True
                        )
                    if not os.path.isdir(params["inpath"]) or not os.path.isdir(
                        params["outpath"]
                    ):
                        log.error(
                            f'Steps animation error: folder not found: {params["inpath"], params["outpath"]}'
                        )
                        return
                    ...
                    if params["ffmpeg"] is None:
                        ...
                        log.error(
                            f"Steps animation error: codec {params['codec']} not supported by ffmpeg"
                        )
                    else:
                        log.info(
                            f'Steps animation creating movie sequence: {params["outfile"]} images={len(imgs)}'
                        )
        ...
        if tmp_delete:
            log.debug(
                f'Steps animation removing {len(temp_files)} files from temp folder: {params["inpath"]}'
            )
            for temp_file in temp_files:
                for fn in pathlib.Path(params["inpath"]).glob(
                    os.path.basename(os.path.splitext(temp_file)[0]) + "*"
                ):
                    fn.unlink()
